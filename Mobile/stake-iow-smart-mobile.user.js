// ==UserScript==
// @name         IOW / Smart (Mobile Userscripts)
// @namespace    http://tampermonkey.net/
// @version      2.1.1
// @description  Userscripts-app mobile counterpart for Stake Dice/Limbo with Manual / IOW / Smart modes, compact controls, and stats.
// @author       .
...
(function () {
    ...
    const SCRIPT_ID = 'iow-smart-mobile-userscripts';
    ...
    function getPlayButton() {
        const direct = document.querySelector('button[data-testid="bet-button"]');
        if (direct && direct.offsetParent !== null) return direct;

        const buttons = Array.from(document.querySelectorAll('button'));
        return buttons.find(btn => {
            const text = (btn.textContent || '').trim().toLowerCase();
            return btn.offsetParent !== null && (text === 'play' || text.includes('play'));
        }) || null;
    }

    function getAmountInput() {
        return document.querySelector('input[data-testid="input-game-amount"]');
    }
    ...
    function getCurrentBet() {
        ...
        const editingValue = document.querySelector('#text-field-container #editing-view-port > div');
        if (editingValue) {
            const value = parseFloat(String(editingValue.textContent || '').replace(/,/g, '').replace(/[^\d.]/g, ''));
            if (Number.isFinite(value)) return value;
        }
    ...
    function setBet(amount) {
        ...
        const editingValue = document.querySelector('#text-field-container #editing-view-port > div');
        if (container && editingValue) {
            editingValue.textContent = target;
            ['input', 'change', 'blur'].forEach(type => {
                container.dispatchEvent(new Event(type, { bubbles: true }));
                editingValue.dispatchEvent(new Event(type, { bubbles: true }));
            });
            return true;
        }
    ...
    function getCurrentBalance() {
        ...
        const activeCurrency = amountInput?.getAttribute('data-bet-amount-active-currency') || '';
        ...
        for (const elem of balanceElems) {
            ...
            const svgIcon = (svg?.getAttribute('data-ds-icon') || '').toUpperCase();
            const title = (sibling.getAttribute('title') || '').toUpperCase();
            if (svgIcon === iconName || title === iconName) {
                state.lastKnownBalance = val;
                return val;
            }
        }

        if (fallbackVal !== null) {
            state.lastKnownBalance = fallbackVal;
            return fallbackVal;
        }
    ...
    function getLatestBetEntry(container = findPastBetsContainer()) {
        ...
        const entry = container.querySelector('button[data-last-bet-index="0"]') || container.querySelector('button');
        if (!entry) return null;

        const id = entry.getAttribute('data-past-bet-id')
            || entry.getAttribute('data-bet-id')
    ...
    function findPastBetsContainer() {
        return document.querySelector('.past-bets');
    }

    function isWinElement(el) {
        if (!el) return false;
        return el.classList.contains('variant-positive') || /\bwin\b/i.test(el.className);
    }

    function getUserSetMultiplier() {
        const isDiceGame = isDice();
    ...
    function getStatusText() {
        ...
        if (state.activeMode === 'iow') {
            return `IOW • Base ${state.baseBet.toFixed(2)} | Wins: ${state.iowWinCounter} | LossStreak: ${state.iowLossCounter}`;
        }
    ...
    function updateUI() {
        ...
        const fields = {
            startBalance: formatNum(state.initialBalance),
            profit: formatNum(state.currentProfit),
            peakBalance: formatNum(state.sessionPeak),
            peakProfit: formatNum(state.highestProfit),
            totalBets: String(state.totalBets),
            wagered: formatNum(state.totalWagered),
        ...
        syncNumber(`${SCRIPT_ID}-baseBet`, state.baseBet, 2);
    ...
    function bindInputHandlers() {
        ...
        on(`${SCRIPT_ID}-baseBet`, 'input', () => {
            state.baseBet = Math.max(0.01, readNumberInput(`${SCRIPT_ID}-baseBet`, state.baseBet));
        });
            ...
            <div id="${SCRIPT_ID}-settings" class="sm-drawer">
                ...
                <div class="mode-iow-only sm-grid sm-hidden" style="margin-top:8px;">
                    <div class="sm-field">
                        ...
                        <input id="${SCRIPT_ID}-baseBet" type="number" min="0.01" step="0.01" value="0.01">
                    </div>
                    ...
                    <div class="sm-field">
                        ...
                        <input id="${SCRIPT_ID}-autoStopBalance" type="number" min="0" step="0.01" placeholder="OFF">
                    </div>
                </div>
                ...
                <div class="sm-footer-note">
                    Mobile script keeps the native Stake mobile controls visible and adds only compact mode control, settings, and stats (full desktop smart logic with dynamic multiplier-based bet sizing).
                </div>
            </div>
    ...
    function bootstrap() {
        if (window.__iowSmartMobileUserscriptsLoaded) return;
        window.__iowSmartMobileUserscriptsLoaded = true;
