// ==UserScript==
// @name         Nuts.gg Auto-Vault Utility (Mobile Userscripts)
// @namespace    Nuts Auto-Vault Utility
// @version      1.0
// @description  Mobile Userscripts counterpart for Nuts.gg Auto-Vault — compact overlay, touch drag, safe-area aware. Automatically moves play-balance profits into the vault by piggybacking on the site's GraphQL WebSocket.
...
(function() {
    ...
    function sendVaultDeposit(amountUnits) {
        return new Promise((resolve, reject) => {
            ...
            try { nutsSocket.send(JSON.stringify(payload)); }
            catch (e) { clearTimeout(timeout); pendingMutation = null; reject(e); }
        });
    }
    ...
    function createUI() {
        ...
        #nuts-autovault-floaty .nv-title {
            ...
            font-size: 13px; color: #bb86fc; letter-spacing: 0.3px;
        }
        ...
        #nuts-autovault-floaty .nv-header-btn:active { color: #fff; background: #3a3a4a; }
        #nuts-autovault-floaty .nv-content {
            padding: 12px; display: flex; flex-direction: column; gap: 10px;
        }
        #nuts-autovault-floaty .nv-row {
            display: flex; align-items: center; justify-content: space-between; gap: 8px;
        }
        #nuts-autovault-floaty .nv-label { color: #a0a0b0; font-size: 13px; }
        #nuts-autovault-floaty input[type="number"] {
            ...
            text-align: right; -webkit-appearance: none; appearance: none; min-height: 36px;
        }
        #nuts-autovault-floaty input[type="number"]:focus { outline: none; border-color: #bb86fc; }
        #nuts-autovault-floaty .nv-btn-row { display: flex; gap: 8px; margin-top: 4px; }
        #nuts-autovault-floaty .nv-btn {
            flex: 1; background: #2a2a3a; color: #d0d0e0; border: 1px solid #3a3a4a;
            border-radius: 6px; padding: 10px 12px; font-size: 12px; font-weight: 700;
            cursor: pointer; text-transform: uppercase; letter-spacing: 0.5px;
            min-height: 40px; -webkit-tap-highlight-color: transparent;
        }
        #nuts-autovault-floaty .nv-btn:active:not(:disabled) { transform: scale(0.97); }
        ...
        #nuts-autovault-floaty .nv-btn.danger {
            background: linear-gradient(135deg, #ff0266, #c2185b); border-color: #ff0266; color: #fff;
        }
        #nuts-autovault-floaty .nv-stats {
            display: flex; justify-content: space-between; gap: 6px;
            padding-top: 10px; border-top: 1px solid #3a3a4a; font-size: 12px;
        }
        #nuts-autovault-floaty .nv-stat { display: flex; flex-direction: column; gap: 3px; }
        #nuts-autovault-floaty .nv-stat-label {
            color: #7a7a8a; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px;
        }
        #nuts-autovault-floaty .nv-stat-value { color: #bb86fc; font-weight: 700; font-size: 13px; }
        #nuts-autovault-floaty .nv-log-toggle {
            display: flex; align-items: center; justify-content: space-between;
            padding: 10px 12px; background: #1e1e2e; border-top: 1px solid #3a3a4a;
            cursor: pointer; min-height: 38px; -webkit-tap-highlight-color: transparent;
        }
        #nuts-autovault-floaty .nv-log-toggle:active { background: #24243a; }
        #nuts-autovault-floaty .nv-log-toggle-text {
            font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600;
        }
        #nuts-autovault-floaty .nv-log-toggle-icon {
            font-size: 11px; color: #7a7a8a; transition: transform 0.2s;
        }
        ...
        #nuts-autovault-floaty .nv-log.open { max-height: 160px; }
        #nuts-autovault-floaty .nv-log-inner {
            padding: 8px 10px; max-height: 160px; overflow-y: auto;
            -webkit-overflow-scrolling: touch;
            font-family: 'Menlo', 'Monaco', 'Consolas', monospace; font-size: 10px; line-height: 1.5;
        }
        ...
        #nuts-autovault-floaty.mini { width: auto; border-radius: 22px; }
        #nuts-autovault-floaty.mini .nv-header { border-radius: 22px; padding: 8px 14px; border-bottom: none; }
        ...
        #nuts-autovault-stealth {
            position: fixed; bottom: calc(env(safe-area-inset-bottom, 0px) + 12px); right: 12px;
            width: 14px; height: 14px; border-radius: 50%; background: #4a5568;
            cursor: pointer; z-index: 999999; box-shadow: 0 2px 6px rgba(0,0,0,0.4);
            -webkit-tap-highlight-color: transparent;
        }
        #nuts-autovault-stealth:active { transform: scale(1.3); }
        ...
        content.innerHTML = `
            <div class="nv-row">
                ...
                <input type="number" id="nvSavePct" min="0" max="1" step="0.01" value="${SAVE_AMOUNT}" inputmode="decimal">
            </div>
            <div class="nv-row">
                <span class="nv-label">Big-win threshold (×)</span>
                <input type="number" id="nvBigWin" min="1" step="0.1" value="${BIG_WIN_THRESHOLD}" inputmode="decimal">
            </div>
            <div class="nv-row">
                <span class="nv-label">Big-win multiplier</span>
                <input type="number" id="nvBigMult" min="1" step="0.1" value="${BIG_WIN_MULTIPLIER}" inputmode="decimal">
            </div>
            <div class="nv-row">
                <span class="nv-label">Check interval (s)</span>
                <input type="number" id="nvCheck" min="10" step="1" value="${Math.round(CHECK_INTERVAL/1000)}" inputmode="numeric">
            </div>
            <div class="nv-row">
                <span class="nv-label">Min deposit (SOL)</span>
                <input type="number" id="nvMinDep" min="0" step="0.0001" value="${MIN_DEPOSIT_SOL}" inputmode="decimal">
            </div>
        ...
        const isHandle = t => {
            if (t.closest('.nv-header-btns')) return false;
            return Boolean(t.closest('[data-drag-handle="true"]'));
        };
        const startDrag = (x, y) => {
            const rect = widget.getBoundingClientRect();
            if (!widget.style.left) { cx = rect.left; cy = rect.top; }
            ix = x - cx; iy = y - cy;
            isDragging = true;
        };
        const dragMove = (x, y) => {
            let nx = x - ix, ny = y - iy;
            nx = Math.max(0, Math.min(window.innerWidth - widget.offsetWidth, nx));
            ny = Math.max(0, Math.min(window.innerHeight - widget.offsetHeight, ny));
            cx = nx; cy = ny;
    ...
    onDomReady(() => {
        setTimeout(() => {
            if (window.__nutsAutovaultMobileLoaded) return;
            window.__nutsAutovaultMobileLoaded = true;
